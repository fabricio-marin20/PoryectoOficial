...
class PersonasDao {
 ...
 public function add(Personas $personas) {
        ...
        global $labAdodb;
        try {
            $sql = sprintf("insert into Personas (PK_cedula, nombre, apellido1, apellido2, fecNacimiento, Correo, Usu, Contrasena, tel, TipoUsuario) 
                    ...
                    $this->labAdodb->Param("Usu"),
                    $this->labAdodb->Param("Contrasena"),
                    $this->labAdodb->Param("tel"),
            ...
            $valores["Usu"]             = $personas->getUsu();
            $valores["Contrasena"]      = $personas->getContrasena();
            $valores["tel"]             = $personas->gettel();
    ...
    public function searchById(Personas $personas) {
        ...
        try {
            ...
            if ($resultSql->RecordCount() > 0) {
                ...
                $returnPersonas->setUsu($resultSql->Fields("Usu"));
                $returnPersonas->setContrasena($resultSql->Fields("Contrasena"));
                $returnPersonas->settel($resultSql->Fields("tel"));
